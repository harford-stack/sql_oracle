...
SELECT ENAME, D.DNAME, SAL, 부서평균
FROM EMP E
INNER JOIN DEPT D ON E.DEPTNO = D.DEPTNO
INNER JOIN (
    SELECT DNAME, ROUND(AVG(SAL)) AS 부서평균
    FROM EMP E
    INNER JOIN DEPT D ON E.DEPTNO = D.DEPTNO
    GROUP BY DNAME
) S ON D.DNAME = S.DNAME
WHERE SAL > 부서평균;
...
SELECT STU_NAME, ROUND(AVG(ENR_GRADE)), 학과평균
FROM STUDENT S
INNER JOIN ENROL E ON S.STU_NO = E.STU_NO
INNER JOIN (
    SELECT STU_DEPT, ROUND(AVG(ENR_GRADE)) 학과평균
    FROM STUDENT S
    INNER JOIN ENROL E ON S.STU_NO = E.STU_NO
    GROUP BY STU_DEPT
) T ON S.STU_DEPT = T.STU_DEPT
GROUP BY STU_NAME, 학과평균
HAVING AVG(ENR_GRADE) > 학과평균;
