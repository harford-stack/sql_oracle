...
-- (ex, 010-****-5678) -- 가운데 숫자는 4글자 고정으로 가정
-- 3. 게시글에 좋아요 개수가 가장 높은 게시글을 작성한 사람의 아이디, 이름, 좋아요개수를 출력하시오.
...
SELECT PHONE, SUBSTR(PHONE, 1, INSTR(PHONE, '-')) || '****' || SUBSTR(PHONE, INSTR(PHONE, '-')+5, LENGTH(PHONE))
FROM TBL_USER;
...
SELECT B.BOARDNO, TITLE, B.CONTENTS, NICKNAME, COUNT(COMMENTNO)
FROM TBL_BOARD B
