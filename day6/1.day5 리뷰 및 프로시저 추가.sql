-- 사용자 정의 함수
CREATE OR REPLACE FUNCTION REVIEW1
    (P_NUMBER IN NUMBER)
RETURN NUMBER -- 문자일때에;는 VARCHAR2
IS
BEGIN
    RETURN P_NUMBER * 10;
END;
...
SELECT ENAME, SAL, REVIEW1(SAL), REVIEW1(10) FROM EMP;
...
BEGIN
    ...
    IF P_COUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('해당 교수번호가 존재하지 않음');
    ELSIF P_COUNT = 1 THEN
        DBMS_OUTPUT.PUT_LINE('정상적으로 수정됨');
    ELSE
        DBMS_OUTPUT.PUT_LINE('2개 이상의 보너스가 수정되었습니다');
    END IF;
    COMMIT;
END;
/
...
CREATE OR REPLACE PROCEDURE HEIGHT_UPDATE
    (
        ...
        P_GROWHEIGHT IN STUDENT.STU_HEIGHT%TYPE
    )
IS
    P_COUNT NUMBER;
BEGIN
    UPDATE STUDENT
    SET
        STU_HEIGHT = STU_HEIGHT + P_GROWHEIGHT
    WHERE STU_NO = P_STUNO;
    
    P_COUNT := SQL%ROWCOUNT;
    IF P_COUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('해당 학생번호가 존재하지 않음');
    ELSIF P_COUNT = 1 THEN
        DBMS_OUTPUT.PUT_LINE('수정되었습니다');
    ELSE
        DBMS_OUTPUT.PUT_LINE('2명 이상의 키가 수정되었습니다');
    END IF;
    COMMIT;
END;
/
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('힉년 : ' || P_GRADE);
END;
/

EXECUTE HEIGHT_UPDATE(20153075);
